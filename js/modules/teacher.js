async function loadTeachersList() {
    try {
        ...
        logError("loadTeachersList error", e.message);
        const container = document.getElementById("teachers-list-container");
        if (container) container.innerHTML = "<p>Erreur chargement enseignants</p>";
    }
}
...
async function addTeacher(fullName, subject, email, phone) {
    try {
        ...
        logError("addTeacher error", e.message);
        throw e;
    }
}
...
async function updateTeacher(id, fullName, subject, email, phone) {
    try {
        const { error } = await supabaseClient
            .from('teachers')
            .update({ full_name: fullName, subject, email, phone })
            .eq('id', id);
        if (error) throw error;
    } catch (e) {
        logError("updateTeacher error", e.message);
        throw e;
    }
}
...
async function deleteTeacher(id) {
    ...
    try {
        ...
        if (error) throw error;
        showAlert("Enseignant supprimé", "success");
        await loadTeachersList();
    } catch (e) {
        logError("deleteTeacher error", e.message);
...
async function saveEditTeacher() {
    try {
        ...
        logError("saveEditTeacher error", e.message);
...
function initTeacherForm() {
    ...
    addBtn.addEventListener("click", async () => {
        try {
            ...
            if (!name.trim()) { showAlert("Le nom de l'enseignant est requis", "error"); return; }
            ...
            logError("initTeacherForm add error", e.message);
            showAlert("Erreur: " + e.message, "error");
        }
    });
            }
