async function loadParentsList() {
    try {
        ...
        logError("loadParentsList error", e.message);
        const container = document.getElementById("parents-list-container");
        if (container) container.innerHTML = "<p>Erreur chargement parents</p>";
    }
}
...
async function addParent(fullName, email, phone) {
    try {
        ...
        logError("addParent error", e.message);
        throw e;
    }
}
...
async function updateParent(id, fullName, email, phone) {
    try {
        ...
        logError("updateParent error", e.message);
        throw e;
    }
}
...
async function deleteParent(id) {
    ...
    try {
        ...
        if (error) throw error;
        showAlert("Parent supprimé", "success");
        await loadParentsList();
    } catch (e) {
        logError("deleteParent error", e.message);
...
async function saveEditParent() {
    try {
        ...
        if (!name.trim()) { showAlert("Le nom est requis", "error"); return; }
        await updateParent(id, name, email, phone);
        showAlert("Parent modifié", "success");
        closeEditParentModal();
        await loadParentsList();
    } catch (e) {
        logError("saveEditParent error", e.message);
...
function initParentForm() {
    ...
    addBtn.addEventListener("click", async () => {
        try {
            ...
            if (!name.trim()) { showAlert("Le nom du parent est requis", "error"); return; }
            ...
            logError("initParentForm add error", e.message);
            showAlert("Erreur: " + e.message, "error");
        }
    });
}
...
async function loadParentsForStudent(studentId) {
    try {
        ...
        logError("loadParentsForStudent error", e.message);
        return [];
    }
}
...
async function linkParentToStudent(parentId, studentId, shouldLink) {
    try {
        ...
        logError("linkParentToStudent error", e.message);
        throw e;
    }
            }
